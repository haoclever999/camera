...
@extends('layouts.admin') @section('title')
    ...
    <div class="input-group timkiem">
        <input type="search" class="form-control bg-light border-0 small" placeholder="Tìm kiếm..." name="timkiem_dh"
            id="timkiem_dh" />
    </div>
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                        ...
                        <form>
                            <div class="form-group">
                                <select class="form-select input-sm" onchange="this.form.submit();" name="sapxep">
                                    <option {{ request('sapxep') == 'mac_dinh' ? 'selected' : '' }} value="mac_dinh"> Mặc
                                        định </option>
                                    <option {{ request('sapxep') == 'cho_xu_ly' ? 'selected' : '' }} value="cho_xu_ly"> Đang
                                        chờ xử lý </option>
                                    <option {{ request('sapxep') == 'xac_nhan' ? 'selected' : '' }} value="xac_nhan"> Đã xác
                                        nhận </option>
                                    <option {{ request('sapxep') == 'van_chuyen' ? 'selected' : '' }} value="van_chuyen">
                                        Đang vận chuyển </option>
                                    <option {{ request('sapxep') == 'giao' ? 'selected' : '' }} value="giao"> Đã giao
                                    </option>
                                    <option {{ request('sapxep') == 'huy' ? 'selected' : '' }} value="huy"> Đã huỷ
                                    </option>
                                    <option {{ request('sapxep') == 'xoa' ? 'selected' : '' }} value="xoa"> Đã xoá
                                    </option>
                                </select>
                            </div>
                        </form>
                    ...
                    <div class="col-sm-8">
                        <form>
                            <div class="form-group row">
                                <label style="padding-top: 1%; width: 12%;" class=" control-label" for="tu_ngay">Từ
                                    ngày</label>
                                <input type="date" class="col-md-3 form-control" name="tu_ngay" id="tu_ngay"
                                    value="{{ request('tu_ngay') ?? '' }}">

                                <label style="padding-top: 1%; width: 14%;" class="control-label" for="den_ngay">Đến
                                    ngày</label>
                                <input type="date" class="col-md-3 form-control" name="den_ngay" id="den_ngay"
                                    value="{{ request('den_ngay') ?? '' }}">

                                <button type="submit" class="col-md-1 btn btn-primary"
                                    style="margin-left: 2%; width: max-content;">
                                    Lọc
                                </button>
                            </div>
                        </form>
                    ...
                    <div class="col-sm-2" style="float: right; margin-top: -1%;">
                        <a href="{{ route('donhang.xuatdonhnag') }}" class="btn btn-success float-right m-2">
                            Xuất excel
                        </a>
                    </div>
                    <!-- /.col -->
                                ...
                                <tbody>
                                    @foreach ($dhang as $dh)
                                        ...
                                        "
                                                ...
                                                <br />
                                                @if (auth()->check() && auth()->user()->quyen == 'Quản trị' && $dh->trang_thai != 'Đã xoá')
                                                    <a style="
                                            min-width: 80px;
                                            padding: 3px 12px;
                                            margin: 3px;
                                        "
                                                        class="btn btn-danger action_del" href=""
