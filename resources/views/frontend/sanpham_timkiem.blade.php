                    ...
                    <form>
                        <div class="pull-right">
                            ...
                            <select class="form-control input-sm" onchange="this.form.submit();" name="hienthi">
                                <option {{ request('hienthi') == '6' ? 'selected' : '' }} value="6"
                        ...
                        <div class="pull-right">
                            ...
                            <select class="form-control input-sm" onchange="this.form.submit();" name="sx_sp">
                                ...
                                <option {{ request('sx_sp') == 'a_z' ? 'selected' : '' }} value="a_z">Tên (A - Z)
                                </option>
                                <option {{ request('sx_sp') == 'z_a' ? 'selected' : '' }} value="z_a">Tên (Z - A)
                                </option>
                                <option {{ request('sx_sp') == 'thap_cao' ? 'selected' : '' }} value="thap_cao">Giá (Thấp
                                    &gt; Cao)
                                </option>
                                <option {{ request('sx_sp') == 'cao_thap' ? 'selected' : '' }} value="cao_thap">Giá (Cao &gt;
                                    Thấp)
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-12">
                            <div class="product-item">
                                <div class="pi-img-wrapper">
                                    <img src="{{ $tk->hinh_anh_chinh }}" class="img-responsive" />
                                    <div>
                                        <a href="{{ $tk->hinh_anh_chinh }}" class="btn btn-default fancybox-button">
                                            Phóng to
                                        </a>
                                ...
                                @if ($tk->giam_gia != 0)
                                    <div style="display: inline;">
                                        ...
                                        <div class="price">
                                            <del>
                                                <i
                                                    style="margin-left: 1em; height: 25px; line-height: 2; vertical-align: middle;">
                                ...
                                @endif
                                <div>
                                    <p class="btn btn-so-sanh" onclick="ThemSoSanh('{{ $tk->id }}')">
                                        So sánh
                                    </p>
                                    <form>
                                        @csrf
                                        <input type="hidden" id="id_sp" value="{{ $tk->id }}" />
                                        <input type="hidden" id="tensp_{{ $tk->id }}"
                                            value="{{ $tk->ten_sp }}" />
                                        <input type="hidden" id="hinhanh_{{ $tk->id }}"
                                            value="{{ $tk->hinh_anh_chinh }}" />
                                        <input type="hidden" id="giaban_{{ $tk->id }}"
                                            value="{{ number_format($tk->gia_ban - ($tk->gia_ban * $tk->giam_gia) / 100, 0, ',', '.') }}" />
                                        <input type="hidden" id="tinhnang_{{ $tk->id }}"
                                            value="{{ $tk->tinh_nang }}" />
                                        <a id="url_{{ $tk->id }}"
                                            href="{{ route('sanpham.chitiet_sp', [$tk->id]) }}"></a>
                                    </form>
