...
@extends('layouts.admin') @section('title')
    ...
    <div class="input-group timkiem">
        <input type="search" class="form-control bg-light border-0 small" placeholder="Tìm kiếm..." name="timkiem_sp"
            id="timkiem_sp" />
    </div>
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                        ...
                        <form>
                            <div class="form-group">
                                <select class="form-select input-sm" onchange="this.form.submit();" name="sapxep">
                                    ...
                                    <option {{ request('sapxep') == 'a_z' ? 'selected' : '' }} value="a_z"> Tên sản phẩm
                                        (A - Z) </option>
                                    <option {{ request('sapxep') == 'z_a' ? 'selected' : '' }} value="z_a"> Tên sản phẩm
                                        (Z - A) </option>
                                    <option {{ request('sapxep') == 'thap_cao' ? 'selected' : '' }} value="thap_cao"> Giá
                                        sản phẩm (Thấp &gt; Cao)
                                    </option>
                                    <option {{ request('sapxep') == 'cao_thap' ? 'selected' : '' }} value="cao_thap"> Giá sản
                                        phẩm (Cao &gt; Thấp)
                                    </option>
                                </select>
                            </div>
                    ...
                    <div class="col-sm-8" style="float: right">
                        <form action="{{ route('sanpham.nhapsp') }}" method="post" enctype="multipart/form-data"
                            style="display: none" id="fnhapexcel" class="float-right m-2">
                            @csrf
                        ...
                        <form action="{{ route('sanpham.nhap_hinhanh') }}" method="post" enctype="multipart/form-data"
                            style="display: none" id="fnhap_hinhanh" class="float-right m-2">
                            @csrf
                            <input type="file" name="file_Hinhanh" id="file_Hinhanh" required />
                            <button type="submit" class="btn btn-primary">
                                Nhập
                            </button>
                        </form>
                        <a class="btn btn-success float-right m-2" id="nhap_hinhanh">
                            Nhập hình ảnh chi tiết
                        </a>
                        <a href="{{ route('sanpham.getThem') }}" class="btn btn-primary float-right m-2" id="themsp">
                            Thêm sản phẩm
                        </a>
                    </div>
                    <!-- /.col -->
            ...
            <div class="container-fluid">
                <div class="row">
                    @if (auth()->check() && auth()->user()->quyen == 'Quản trị')
                        @if ($sp_daxoa->count() > 0)
                            <div>
                                <a href="{{ route('sanpham.daxoa') }}" class="btn btn-danger float-left m-2" id="delete">
                                    Đã xóa
                                </a>
                            </div>
                        @endif
