@extends('layouts.admin') @section('title')
    ...
    <div class="input-group timkiem">
        <input type="search" class="form-control bg-light border-0 small" placeholder="Tìm kiếm..." name="timkiem_nd"
            id="timkiem_nd" />
    </div>
    @endsection @section('content')
    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper content container-fluid" style="padding-left: 10px; padding-right: 10px">
        <div class="row">
            <div class="col-md-12">
                <a href="{{ route('nguoidung.getThem') }}" class="btn btn-primary float-right m-2">
                    Thêm người dùng
                </a>
            </div>
            <div class="tbl-fixed">
                        ...
                        <tbody>
                            @foreach ($users as $u)
                                <tr>
                                    ...
                                    <td>{{ $u->quyen }}</td>
                                    <td>
                                        {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $u->updated_at)->format('H:i:s d/m/Y') }}
                                    </td>
                                    @if ($u->trang_thai == 1)
                                        <td>Kích hoạt</td>
                                    @else
                                        <td>Khóa</td>
                                        @endif @if ($u->quyen != 'Quản trị')
                                            @if ($u->trang_thai == 1)
                                                <td>
                                                    <form action="{{ route('nguoidung.trangthai', ['id' => $u->id]) }}"
                                                        method="post">
                                                        @csrf
                                                        <input type="hidden" name="khoa" value="0" />
                                                        <button type="submit" class="btn btn-danger action_edit">
                                                            Khóa
                                                        </button>
                                                    </form>
                                                </td>
                                        ...
                                        @endif
                                        <td>
                                            <form action="{{ route('nguoidung.postcapnhatquyen', ['id' => $u->id]) }}"
                                                method="post" style="display: none" class="fcapnhatquyen">
                                                @csrf
                                                <select id="opt_quyen" name="opt_quyen">
                                                    <option
                                                        {{ $u->quyen == 'Quản trị' ? 'selected' : '' }}
                                                        value="Quản trị">Quản trị
                                                    </option>
                                                    <option
                                                        {{ $u->quyen == 'Nhân viên' ? 'selected' : '' }}
                                                        value="Nhân viên">Nhân viên
                                                    </option>
                                                    <option
                                                        {{ $u->quyen == 'Khách hàng' ? 'selected' : '' }}
                                                        value="Khách hàng">Khách hàng
                                                    </option>
                                                </select>
