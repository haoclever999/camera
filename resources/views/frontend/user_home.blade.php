                ...
                <div class="col-md-3" style="margin-top: 6px">
                    <div class="product-item">
                        <div class="pi-img-wrapper">
                            <img src="{{ $sp_m->hinh_anh_chinh }}" class="img-responsive" />
                            <div>
                                <a href="{{ $sp_m->hinh_anh_chinh }}" class="btn btn-default fancybox-button">
                                    Phóng to
                                </a>
                        ...
                        @if ($sp_m->giam_gia != 0)
                            <div style="display: inline">
                                <div class="pi-price">
                                    {{ number_format($sp_m->gia_ban - ($sp_m->gia_ban * $sp_m->giam_gia) / 100, 0, ',', '.') }}đ
                                </div>
                                <div class="price">
                                    <del>
                                        <i
                        ...
                        @endif
                        <div>
                            <p class="btn btn-so-sanh" onclick="ThemSoSanh('{{ $sp_m->id }}')">
                                So sánh
                            </p>
                            <form>
                                @csrf
                                <input type="hidden" id="id_sp" value="{{ $sp_m->id }}" />
                                <input type="hidden" id="tensp_{{ $sp_m->id }}" value="{{ $sp_m->ten_sp }}" />
                                <input type="hidden" id="hinhanh_{{ $sp_m->id }}"
                                    value="{{ $sp_m->hinh_anh_chinh }}" />
                                <input type="hidden" id="giaban_{{ $sp_m->id }}"
                                    value="{{ number_format($sp_m->gia_ban - ($sp_m->gia_ban * $sp_m->giam_gia) / 100, 0, ',', '.') }}" />
                                <input type="hidden" id="tinhnang_{{ $sp_m->id }}" value="{{ $sp_m->tinh_nang }}" />
                                <a id="url_{{ $sp_m->id }}" href="{{ route('sanpham.chitiet_sp', [$sp_m->id]) }}"></a>
                            </form>
        ...
        <div class="col-md-12 sale-product">
            ...
            @foreach ($sp_noi_bat as $sp_nb)
                <div class="col-md-3" style="margin-top: 6px">
                    <div class="product-item">
                        <div class="pi-img-wrapper">
                            <img src="{{ $sp_nb->hinh_anh_chinh }}" class="img-responsive" />
                            <div>
                                <a href="{{ $sp_nb->hinh_anh_chinh }}" class="btn btn-default fancybox-button">
                                    Phóng to
                                </a>
                        ...
                        @if ($sp_nb->giam_gia != 0)
                            <div style="display: inline">
                                <div class="pi-price">
                                    {{ number_format($sp_nb->gia_ban - ($sp_nb->gia_ban * $sp_nb->giam_gia) / 100, 0, ',', '.') }}đ
                                </div>
                                <div class="price">
                                    <del>
                                        <i
                        ...
                        @endif
                        <div>
                            <p class="btn btn-so-sanh" onclick="ThemSoSanh('{{ $sp_nb->id }}')">
                                So sánh
                            </p>
                            <form>
                                @csrf
                                <input type="hidden" id="id_sp" value="{{ $sp_nb->id }}" />
                                <input type="hidden" id="tensp_{{ $sp_nb->id }}" value="{{ $sp_nb->ten_sp }}" />
                                <input type="hidden" id="hinhanh_{{ $sp_nb->id }}"
                                    value="{{ $sp_nb->hinh_anh_chinh }}" />
                                <input type="hidden" id="giaban_{{ $sp_nb->id }}"
                                    value="{{ number_format($sp_nb->gia_ban - ($sp_nb->gia_ban * $sp_nb->giam_gia) / 100, 0, ',', '.') }}" />
                                <input type="hidden" id="tinhnang_{{ $sp_nb->id }}" value="{{ $sp_nb->tinh_nang }}" />
                                <a id="url_{{ $sp_nb->id }}"
                                    href="{{ route('sanpham.chitiet_sp', [$sp_nb->id]) }}"></a>
                            </form>
                            <form action="{{ route('giohang.them_giohang') }}" method="post">
                                @csrf
